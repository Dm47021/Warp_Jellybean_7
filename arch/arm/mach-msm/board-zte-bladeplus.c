...
#if defined(CONFIG_TOUCHSCREEN_FOCALTECH_NEW)
#include <linux/input/ft5x0x_ts_new.h>
#endif
...
#define MSM_RAM_CONSOLE_PHYS  0x3D00000 //// Change memory map for kernel 4M alignment , 
...
static struct pmic8058_keypad_data surf_keypad_data = {
	.input_name		= "bladeplus_keypad",
	.input_phys_device	= "bladeplus_keypad/input0",
	.num_rows		= 12,
	.num_cols		= 8,
	.rows_gpio_start	= 8,
	.cols_gpio_start	= 0,
...
/* add camera sensor I2C  begin */
    ...
    /*
     * add by for MT9D115-2.0Mp-FF-Socket
     *
     * Attention: I2C device is initialized in sensor's driver if "CONFIG_SENSOR_ADAPTER"
     *            is defined
     */
...
#endif
#endif
/* add sensor I2C info  end */
};

#if 1

#ifdef CONFIG_MSM_CAMERA
/* Config camera GPIO  begin */ 
...
/* add sensor init data structure begin*/
#ifdef CONFIG_MT9P111
/*
 * Commented by zhang.shengjie
 *
...
/* add sensor init data structure end */
#endif
#ifdef CONFIG_OV5640
/*
 * Commented by zhang.shengjie
...
#if defined( CONFIG_MACH_V9PLUS) //  set max_x and max_y for V9 screen
...
int32_t msm_sensors_power_supply(void)
{
	...
	rc_vdd = vreg_set_level(vreg_l10_2p6, 3000);
    if (rc_vdd)
    {
		printk(KERN_ERR"%s: vreg_set_level failed!\n", __func__);
		return -EIO;
...
static int mbp_setup_vregs(int state)
{
	...
	if (state) {
		rc = vreg_set_level(vreg_analog, 3000);
		if (rc) {
...
MACHINE_START(BLADEPLUS, "bladeplus")
#ifdef CONFIG_MSM_DEBUG_UART
	.phys_io  = MSM_DEBUG_UART_PHYS,
	.io_pg_offst = ((MSM_DEBUG_UART_BASE) >> 18) & 0xfffc,
#endif
