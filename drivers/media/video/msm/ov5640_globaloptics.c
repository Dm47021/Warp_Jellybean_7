 ...
 */
#ifdef CONFIG_ZTE_PLATFORM
//#ifdef CONFIG_ZTE_FLASH_AUTO_FLAG_SUPPORT
...
static int32_t ov5640_set_iso(int8_t iso_val)
{
    ...
    switch (iso_val)
    {
        ...
        case CAMERA_ISO_SET_800:
        {
    	    ...
    	    msleep(100);	
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3A18 ,0x0000, WORD_LEN);//ZTE_YGL_CAM_20110711,modifed for SNR
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3A19 ,0x00f8, WORD_LEN);//ZTE_YGL_CAM_20110711,modifed for SNR
            if (rc < 0)
            {
               return rc;
            }
...
static long ov5640_hw_ae_parameter_record(void)
{
    ...
    //kenxu freeze awb 20110519
    rc = ov5640_i2c_write(ov5640_client->addr, 0x5196, 0x0023, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }    
...
//ZTE_YGL_CAM_20110711,modifed for SNR
...
/*
* add flash LED Auto mode flag judgement of ov5640,GYL_CAM_20110718
*/
...
uint16_t ov5640_auto_flash_threshold = 0xF0; //added by GYL_CAM_20110718 for adding flash LED Auto mode flag judgement 
 ...
 /*
  *added by GYL_CAM_20110718 for adding flash LED Auto mode flag judgement 
  */
...
static int zte_flash_on_fix_wb_setreg()
{
	...
	int rc = 0;
	pr_err("%s\r\n",__func__);
	rc = ov5640_i2c_write(ov5640_client->addr, 0x3406, 0x01, WORD_LEN); 
	if (rc < 0)
	{
		return rc;
	} 
	...
	rc = ov5640_i2c_write(ov5640_client->addr, 0x3400, 0x07, WORD_LEN); //0x0a
	if (rc < 0)
	{
		return rc;
	} 
   rc = ov5640_i2c_write(ov5640_client->addr, 0x3401, 0x5C, WORD_LEN);
	if (rc < 0)
	{
		return rc;
	} 
   rc = ov5640_i2c_write(ov5640_client->addr, 0x3402, 0x04, WORD_LEN); //0x05
	if (rc < 0)
	{
		return rc;
	} 
   rc = ov5640_i2c_write(ov5640_client->addr, 0x3403, 0x50, WORD_LEN);
	if (rc < 0)
	{
		return rc;
	} 
   rc = ov5640_i2c_write(ov5640_client->addr, 0x3404, 0x05, WORD_LEN);
	if (rc < 0)
	{
		return rc;
	} 
   rc = ov5640_i2c_write(ov5640_client->addr, 0x3405, 0xC0, WORD_LEN);
	if (rc < 0)
	{
		return rc;
	} 	
...
//ZTE_YGL_CAM_20110711,modifed for SNR
          if(gain > 122) //reach to max gain 16x, then improve contrast to pass SNR test under low light
          {
			gain = gain / 2;
          	capture_exposure = capture_exposure * 2;	
          	
          	//extend vts to set more exposure time
            rc = ov5640_i2c_write(ov5640_client->addr, 0x380e, 0x0a, WORD_LEN);
            if (rc < 0)
            {
              return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x380f, 0x40, WORD_LEN);
            if (rc < 0)
            {
              return rc;
            }            
		  
          	pr_err("ov5640_hw_ae_transfer:gain33333 =%x \n",gain);     	
          	if(YAVG < 9)
          	{
	...
	//Kenxu add to solve capture brightness not same random ZTE_YGL_20110809
	mdelay(250); //250
...
static long ov5640_set_sensor_mode(int32_t mode)
{
    ...
    switch (mode)
    {
        case SENSOR_PREVIEW_MODE:
        {
			...
			{
				rc = ov5640_i2c_write(ov5640_client->addr, 0x3406, 0x0000, WORD_LEN); //denable wb_auto
