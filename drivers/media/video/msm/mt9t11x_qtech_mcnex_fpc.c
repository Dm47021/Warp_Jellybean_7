...
/*-----------------------------------------------------------------------------------------
  ...
  --------     ----         -------------------------------------    ----------------------
  2011-10-09   gyl         solve the problem of WB                  ZTE_CAM_GYL_20111009
  2011-03-24   lijing       add mclk switch for v9plus               ZTE_CAM_LJ_20110324
  2010-12-16   zt           increase time delay for AF trigger;      ZTE_ZT_CAM_20101216
                            remove the process of sharpness
                            which is decided by brightness value
  2010-12-15   jia.jia      add support for exposure compensation    ZTE_CAM_JIA_20101215
  ...
  2010-12-01   lijing       add refresh result judgement for         ZTE_CAM_LJ_20101129
                            parameters that need to refresh(at the
                            end of setting write 0x8400 register to
                            refresh)
  2010-11-22   zt           disable the AF motor control IC before   ZTE_ZT_CAM_20101122
                            entering the standby status
  2010-10-15   lijing       remove "CONFIG_MACH_V9" for MCLK switch  ZTE_LJ_CAM_20101015
                            which doesn't exit on MSM7x30 board-v9+
  2010-09-08   jia          add exception process of i2c_del_driver  ZTE_JIA_CAM_20100908
  2010-09-06   jia          add support of CAM_CLK_SWT for MCLK      JIA_CAM_20100906
                            switch between back-end and front-end
                            camera
  2010-08-25   zt           reduce the sleep current                 ZT_CAM_20100825
                            for CONFIG_SENSOR_ADAPTER
  2010-08-19   jia          add support for CONFIG_SENSOR_ADAPTER    JIA_CAM_20100819
  2010-08-09   jia          use lens_for_indoor as default setting   JIA_CAM_20100809
                            of lensshading
  ...
  2010-05-15   jia          ignore "rc" of queue_work                ZTE_MSM_CAMERA_JIA_001
  ...
  2010-04-17   jia          Add process of MCLK switch for different ZTE_MSM_CAMERA_JIA_001
                            board types
  2010-04-15   zh.shj       Add process for different sensors,       ZTE_MSM_CAMERA_ZHSHJ_001
  ...
  2010-04-06   jia          Enable MT9T11X_SENSOR_PROBE_INIT         ZTE_MSM_CAMERA_JIA_001
...
#define MT9T11X_CAMIO_MCLK  48000000    //PCLK=48MHz, set by PLL setting
...
#define MT9T11X_GPIO_SWITCH_CTL     181
...
static int32_t mt9t11x_af_trigger(void)
{
    ...
    {       
        ...
        /* ZTE_ZT_CAM_20101216
         * increase time delay from 20ms to 70ms
         */
        mdelay(70);
    }
...
static int32_t mt9t11x_set_wb(int8_t wb_mode)
{
    ...
    switch (wb_mode)
    {
        case CAMERA_WB_MODE_AWB:
        {
           ...
           /*modified by ZTE_CAM_GYL_20111009 to solve the problem of WB(micron_jiang)*/
    ...
    /*
      ...
      */
    //mdelay(100);//deleted by ZTE_CAM_GYL_20111009 to solve the problem of WB(micron_jiang)
...
static int32_t mt9t11x_set_sharpness(int8_t sharpness)
{
    ...
    /* ZTE_ZT_CAM_20101216
     * The judgment of brightness is disused,
     * therefore, current level of sharpness is total 5 level(0~4).
     */
#if 0
 ...
 */
#if defined (CONFIG_MACH_V9PLUS)
    rc = msm_camera_clk_switch(mt9t11x_ctrl->sensordata, MT9T11X_GPIO_SWITCH_CTL, 0);
    if (rc < 0)
    {
        CCRT("%s: camera_clk_switch failed!\n", __func__);
        goto sensor_init_fail;
...
static int mt9t11x_sensor_release_internal(void)
{
    ...
    /*
      * Save values of other registers before enterring hard standby mode
    ...
    /* ZTE_ZT_CAM_20101122
     * Disable the AF motor control IC before entering the standby status
     */
    /* Pull down the GPIO 0 of mt9t11x IC to disable the AF motor control IC */
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x0604, 0x0F00, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    ...
    sys_ctl = 0;
    rc = mt9t11x_i2c_read(mt9t11x_client->addr, 0x001A, &sys_ctl, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    /* SYSCTL: 0x001A
      * bit[4]: 0: GPIOs are powered up in standby
      *         1: GPIOs are powered down in standby
      */
    sys_ctl &= 0xFFEF;
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x001A, sys_ctl, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
 ...
 */
    ...
    rc = msm_camera_drv_start(pdev_wq, mt9t11x_sensor_probe,0);
#else
