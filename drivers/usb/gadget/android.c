...
static void clear_switch4linux_flag(void);
static int usb_cdrom_is_enable(void);
static int linux_has_cdrom(void);
...
struct android_dev {
	...
	int product_id;
	int version;
	///added by cuihaijian  three lines 
...
static int is_cdrom_enabled_for_linux(void)
{
	return zte_usb_parameters.cdrom_enable_for_linux;
}
static void enable_cdrom_for_linux(int enable)
{
	zte_usb_parameters.cdrom_enable_for_linux = !!enable;
}

static int get_disk_luns(void)
{
	return zte_usb_parameters.disk_luns;
}
static void set_disk_luns(int num)
{
	if (num <= 0 || num > 7) {
		zte_usb_parameters.disk_luns = 1;
		return ;
	}
	zte_usb_parameters.disk_luns = num;
	return ;
}
...
#define PARAMETER_FUN(parameter)                                                \
static void set_android_usb_##parameter(char * param)                                    \
{                                                                                        \
	strncpy(zte_usb_parameters.p_##parameter,			\
		param,                                                                   \
...
PARAMETER_FUN(unique_sn);

#include "../../../arch/arm/mach-msm/proc_comm.h"

static int
get_mac_address(unsigned char *addr)
{
	int rc = 0;
	...
	if(!rc) {
		addr[5] = (unsigned char)((data2>>8)&0xff); 
		addr[4] = (unsigned char)(data2&0xff); 
 		addr[3] = (unsigned char)((data1>>24)&0xff); 
		addr[2] = (unsigned char)((data1>>16)&0xff); 
		addr[1] = (unsigned char)((data1>>8)&0xff); 
		addr[0] = (unsigned char)(data1&0xff);
		return 0;
	} else {
		return -1;
	}
...
static int sn_is_unique(void)
{
	return zte_usb_parameters.sn_unique;
}
static void set_sn_unique(int unique)
{
	zte_usb_parameters.sn_unique = unique;
	return ;
}


static ssize_t msm_hsusb_show_unique_sn(struct device *dev,
				  struct device_attribute *attr,
				  char *buf)
{
	int i = 0;

	i = scnprintf(buf, PAGE_SIZE, "%s\n", get_android_usb_unique_sn());
	return i;
}

static ssize_t msm_hsusb_set_sn_unique(struct device *dev,
                                   struct device_attribute *attr,
                                   const char *buf, size_t size)
{
	int value;
	...
	char unique_sn[64] = {0};
	sscanf(buf, "%d", &value);
	printk(KERN_ERR"usb: %s, %d: %d unique sn is %s\n",
	       __FUNCTION__, __LINE__, value, value?"enabled":"disabled");
	set_sn_unique(value);
	if (value) {
		get_mac_address(addr);
		snprintf(unique_sn, sizeof(unique_sn), "%s%02X%02X%02X",
			 get_android_usb_serialnumber(), addr[3], addr[4], addr[5]);
		set_android_usb_unique_sn(unique_sn);
	}
	return size;
}

static ssize_t msm_hsusb_set_sn_unique_rand(struct device *dev,
                                   struct device_attribute *attr,
                                   const char *buf, size_t size)
{
	char unique_sn[32] = {0};
	char unique_sn_rand[32] = {0};
	if (sizeof(unique_sn) < size) {
		return -1;
	}
	memcpy(unique_sn, buf, size);
	unique_sn[sizeof(unique_sn) - 1] = 0;
	
	printk(KERN_ERR"usb: %s, %d: unique sn is %s\n",
	       __FUNCTION__, __LINE__, unique_sn);
	set_sn_unique(1);

	snprintf(unique_sn_rand, sizeof(unique_sn_rand), "%s%s",
		 get_android_usb_serialnumber(), unique_sn);
	set_android_usb_unique_sn(unique_sn_rand);
...
static ssize_t store_enable_cdrom_for_linux(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t size)
{
       unsigned long cdrom_enable = 0;
      	if (!strict_strtoul(buf, 16, &cdrom_enable)) {
		enable_cdrom_for_linux((cdrom_enable == 1)? 1 : 0);
		pr_info("%s: Requested enable_cdrom_for_linux = %d 0x%lx\n",
			__func__, g_enable_cdrom, cdrom_enable);	
	} else {
		pr_info("%s: strict_strtoul conversion failed, %s\n", __func__, buf);
	}
...
static ssize_t show_enable_cdrom_for_linux(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	int i = 0;	
	i = scnprintf(buf, PAGE_SIZE, "is cdrom enabled for linux? [%s]\n",
		      is_cdrom_enabled_for_linux()?"enable":"disable");	
	return i;
}


static ssize_t msm_hsusb_show_disk_luns(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	int i = 0;	
	i = scnprintf(buf, PAGE_SIZE, "disk luns: %d\n",
		      get_disk_luns());	
	return i;
}

static ssize_t msm_hsusb_set_diskluns(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t size)
{
       unsigned long disk_luns = 0;
      	if (!strict_strtoul(buf, 16, &disk_luns)) {
		set_disk_luns(disk_luns);
		pr_info("usb: %s: %d disk_luns %d %ld\n",
			__func__, __LINE__, get_disk_luns(), disk_luns);	
	} else {
		pr_info("%s: strict_strtoul conversion failed, %s\n", __func__, buf);
	}
	return size;
...
static DEVICE_ATTR(enable_cdrom_for_linux, 0664,
		   show_enable_cdrom_for_linux, store_enable_cdrom_for_linux);
static DEVICE_ATTR(disk_luns, 0664,
		   msm_hsusb_show_disk_luns, msm_hsusb_set_diskluns);
static DEVICE_ATTR(unique_sn, 0664,
		   msm_hsusb_show_unique_sn, msm_hsusb_set_sn_unique);
static DEVICE_ATTR(unique_sn_rand, 0664,
		   msm_hsusb_show_unique_sn, msm_hsusb_set_sn_unique_rand);
...
static struct attribute *android_attrs[] = {
	...
	&dev_attr_unique_sn.attr,
	&dev_attr_unique_sn_rand.attr,
	&dev_attr_enable_cdrom.attr,
	&dev_attr_enable_cdrom_for_linux.attr,
	&dev_attr_disk_luns.attr,
...
{
	if (usb_cdrom_is_enable() || is_cdrom_enabled_after_switch() || linux_has_cdrom()) {
		return get_disk_luns() + 1;
	}
	return get_disk_luns();		
}
...
struct fsg_config* init_fsg_cfg(void)
{
	...
	fsg_cfg.can_stall = 0;
	for (i = 0; i < get_disk_luns() && i < FSG_MAX_LUNS; i++) {
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	if (!is_ftm_mode() && !is_pid_configed_from_nv()) {
		...
		if (id < 0)
			return id;
		if (sn_is_unique() && !usb_cdrom_is_enable()) {
...
void schedule_cdrom_stop(void)
{
	...
	global_usbwork.switch_for_linux = 0;
...
void schedule_linux_os(void)
{
	...
	global_usbwork.switch_for_linux = 1;
...
{
	struct usb_ex_work *p = &global_usbwork;
	int switch_for_linux = p->switch_for_linux;
	//p->switch_for_linux = 0;
	return is_cdrom_enabled_for_linux()?(switch_for_linux) : 0;
}
