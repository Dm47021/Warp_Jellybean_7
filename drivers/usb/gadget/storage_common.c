...
static int fsg_get_toc(struct fsg_lun *curlun, int msf, int format, u8 *buf, int start_track)
{
	int i, len;
	switch (format) {
	case 0:
		/* Formatted TOC for all*/
		...
		else if(start_track == SCSI_FLAG_SESSION_LEAD_OUT)
		{	
			buf[5] = 0x14;		/* lead-out track number */
			buf[6] = 0xaa;		/* lead-out track address */
			store_cdrom_address(&buf[8], msf, curlun->num_sectors);
		}
		buf[13] = 0x14;		/* Lead-out track is data */
	...
	case 1:
		...
		buf[5] = 0x14;		/* lead-out track number */
		buf[6] = 0x01;		/* lead-out track address */
		store_cdrom_address(&buf[8], msf, curlun->num_sectors);
