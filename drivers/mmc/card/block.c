...
#include <asm/delay.h>	/* for CPRM dependant delay (austin.2011-04-05) */
#endif
...
#ifdef CONFIG_ZTE_DTV
static int mmc_blk_ioctl_cprm(struct block_device *bdev, fmode_t mode, unsigned int ioc_cmd, unsigned long ioc_arg);	/* for CPRM (austin.2011-04-05) */
#endif
...
static inline void set_card_removed(struct mmc_blk_data *md)
{
	md->card_removed = 1;
}

static inline int is_card_removed(struct mmc_blk_data *md)
{
	return 	md->card_removed;
}
...
#ifdef CONFIG_ZTE_DTV
	.ioctl			= mmc_blk_ioctl_cprm,	/* for CPRM (austin.2011-04-05) */
#endif
	.owner			= THIS_MODULE,
};
...
static int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
   ...
   //jzq only for sd not for emmc
   if(mmc_card_sd(card)){
	if (get_err_times(md) > MAX_ERR_TIMES || is_card_removed(md)) {
	...
	do {
		...
		if (brq.cmd.error || brq.data.error || brq.stop.error) {
			...
			if (err) {
				//sdcard is removed?
			   printk(KERN_ERR "rms:%s: (%d) the card is removed? %d %s %d\n",
				       __func__, err, brq.data.blocks,
				  rq_data_dir(req) == READ?"read":"write", __LINE__);
					   set_card_removed(md);
				goto cmd_err;
			} else {
			//ruanmeisi_20100902
...
#ifdef CONFIG_MMC_BLOCK_DEFERRED_RESUME
    // if (mmc_card_sd(card) || mmc_card_mmc(card))               //jzq add 20110906
...
#endif                   //jzq only for sd not for emmc, emmc delay
...
#ifdef CONFIG_ZTE_DTV
/* for CPRM (austin.2011-04-05) */
static int mmc_blk_ioctl_cprm(struct block_device *bdev, fmode_t mode, unsigned int ioc_cmd, unsigned long ioc_arg)
{
